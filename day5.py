...
def getSeat(fn):
    # Read everyting in 1 go
    df = pd.read_fwf(fn, header=None, widths=[1]*10).to_numpy()

    # F=0, B=1, L=0, R=1
    df = np.where(np.logical_or(df=='F',df=='L'), 0, 1)

    # Set up an array to multiply this grid by which columns are form 2^k
    datRow = 2**np.array([6,5,4,3,2,1,0,2,1,0])
    multArr = np.tile(datRow, (df.shape[0],1))

    # Multiply together
    datArr = df*multArr

    # We then have two different bits of data, row and column
    rows = np.sum(datArr[:,:7], axis=1)
    cols = np.sum(datArr[:,7:], axis=1)

    # Seat IDs:
    seatIDs = np.sort(rows*8 + cols)
...
if __name__ == '__main__':
    ...
    # Our seat is the missing one, which will mean the diff in seat number is >1
    missID = np.diff(np.sort(IDs)).argmax()
    # Our Seat
    print(IDs[missID]+1)
