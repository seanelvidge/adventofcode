...
######
This is *really* ugly. But couldn't think of a better way
"""
import numpy as np
import pandas as pd

def validPassports(fn):
    valid = 0
    with open(fn) as f:
        lines = f.readlines()

    lines.insert(0,'\n')
    entrySep = np.where(np.array(lines) == '\n')[0]

    for ent in np.arange(len(entrySep)-1):
        tmp = ''.join(lines[entrySep[ent]:entrySep[ent+1]]).replace('\n',' ')
        # -2 for the extra \n we added to the start/end
        numEntries = len(tmp.split(' ')) - 2

        if numEntries == 8:
            valid += 1
        elif (numEntries == 7) and ('cid:' not in tmp):
            valid += 1
        else:
            pass

    # One last entry to check
    tmp = ''.join(lines[entrySep[-1]:]).replace('\n',' ')
    # -2 for the extra \n we added to the start/end
    numEntries = len(tmp.split(' ')) - 2
...
def validPassportsCheck(fn):
    ...
    for ent in np.arange(len(entrySep)-1):
        ...
        # -2 for the extra \n we added to the start/end
        numEntries = len(tmp.split(' ')) - 2
        passData = tmp.split(' ')[1:-1]

        if numEntries == 8:
            valid += checkCriteria(passData)
        elif (numEntries == 7) and ('cid:' not in tmp):
            valid += checkCriteria(passData)
        else:
            pass
...
def checkCriteria(line):
    validLine = 1

    for term in line:
        crit,num = term.split(':')
        if crit == 'byr': validLine *= byr(num)
        if crit == 'iyr': validLine *= iyr(num)
        if crit == 'eyr': validLine *= eyr(num)
        if crit == 'hgt': validLine *= hgt(num)
        if crit == 'hcl': validLine *= hcl(num)
        if crit == 'ecl': validLine *= ecl(num)
        if crit == 'pid': validLine *= pid(num)

    return validLine


def byr(num):
    return 1920 <= np.int(num) <= 2002

def iyr(num):
    return 2010 <= np.int(num) <= 2020

def eyr(num):
    return 2020 <= np.int(num) <= 2030

def hgt(num):
    if num[-2:] == 'in':
        return 59 <= np.int(num[:-2]) <= 76
    elif num[-2:] == 'cm':
        return 150 <= np.int(num[:-2]) <= 193
    else:
        return False
...
def ecl(num):
    if num in ['amb','blu','brn','gry','grn','hzl','oth']:
        return True
    else:
        return False
...
if __name__ == "__main__":
    fn = 'day4.dat'
    # fn = 'demo.dat'
    print(validPassports(fn))
    print(validPassportsCheck(fn))
